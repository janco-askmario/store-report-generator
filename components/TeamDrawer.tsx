...
export function TeamDrawer({
  ...
  /*
   * The drawer is portalled to <body>, and has to be.
   *
   * Both headers are `backdrop-blur`, and a backdrop-filter makes an element the
   * containing block for every fixed-position descendant — so rendered in place,
   * `fixed inset-0` would resolve against the header's own box and the drawer
   * would be squashed into the top bar instead of covering the page. The portal
   * takes it out of that box, and out of the header's stacking context with it.
   */
  ...
  return (
    <>
      ...
      {canPortal &&
        createPortal(
          /* Kept in the DOM while closed so opening animates rather than
             appears. z-50 clears the sticky headers (z-20) on both pages. */
          <div
